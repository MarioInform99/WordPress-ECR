...
if ( ! get_theme_mod( 'single_post_navigation', webion_theme()->customizer->get_default( 'single_post_navigation' ) ) ) {
	return;
}
...
if ( $the_query->have_posts() ) { ?>
	...
	<nav class="navigation post-navigation">
		<h2 class="screen-reader-text"><?php echo esc_html__( 'Post navigation', 'webion' ); ?></h2>
		...
		<div class="nav-links">
			...
			while ( $the_query->have_posts() ) :
				...
				if ( $previous_post && get_the_ID() === $previous_post->ID ) {
					$nav_label 		= esc_html__( 'Prev Post', 'webion' );
					$nav_class 		.= 'prev';
				} elseif ( $next_post && get_the_ID() === $next_post->ID ) {
					$nav_label 		= esc_html__( 'Next Post', 'webion' );
					$nav_class 		.= 'next';
				}
				...
				<div class="<?php echo esc_attr( $nav_classes ); ?>">
					<a class="nav-links__item" href="<?php echo esc_url( $permalink ); ?>">
						<span>
							<?php echo webion_get_icon_svg( 'post-' . esc_attr( $nav_class ) ) ?>
